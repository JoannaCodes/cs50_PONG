--With Sound, Function Fixes, Resize, AI Implemetation
...
function love.update(dt)
    ...
    --Player 1 AI Implementation
    if gamestate == 'play' then
        --if ball bounced to the left (to Player 1)..
        if ball.dx < 0 then
            --It will check if the paddle is over the ball then
            if Player1.y > ball.y then
                Player1.dy = -PADDLE_SPEED --it will go up
                if ball.y < VIRTUAL_WIDTH / 4 then
                    Player1.dy = (-PADDLE_SPEED * 0.75) + 20
                end
